...
<%
	...
	//    상품정보 조회(상품정보+카테고리+제조회사+상태)
	Product product = productDao.getProductByNo(productNo);
	//    상품의 추가혜택정보 조회
	List<Benefit> productBenefits = productDao.getBenefitsByProductNo(productNo);
...
%>
				<option value="<%=company.getNo() %>" <%=product.hasCompany(company.getNo()) ? "selected" : "" %>> <%=company.getName() %></option>
<%
	}
%>
			</select>
...
%>
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="statusNo" value="<%=status.getNo() %>" <%=product.hasStatus(status.getNo()) ? "checked" : "" %>>
...
%>
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="checkbox" name="benefitNo" value="<%=benefit.getNo() %>" <%=product.hasBenefit(benefit.getNo()) ? "checked" : "" %>>
