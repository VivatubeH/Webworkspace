...
<body>
	...
	<!-- 
	 ...
	 -->
<%
	// 요청파라미터에서 부서 아이디를 조회한다.
	String value = request.getParameter("id");
	// 조회된 부서아이디를 정수로 변환한다.
	int deptId = Integer.parseInt(value);
	
	// 부서상세정보를 제공하는 기능이 구현된 DeptDao 객체를 생성한다.
	DeptDao deptDao = new DeptDao();
	// 부서상세정보를 제공하는 getDeptDetail 메소드를 실행한다.
	Optional<DeptDetail> optional = deptDao.getDeptDetail(deptId);
	
	// 직원목록정보를 제공하는 기능이 구현된 EmpDao 객체를 생성한다.	
	EmpDao empDao = new EmpDao();
	// 직원목록정보를 제공하는 getEmployessByDeptId 메소드를 실행한다.
	List<Emp> employees = empDao.getEmployeesByDeptId(deptId);
%>
