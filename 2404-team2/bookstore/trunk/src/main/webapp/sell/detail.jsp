        ...
        <div class="col-12">
            ...
            <table class="table">
                ...
                <tbody class="align-middle">
					<tr>
	                    <td rowspan="5"><img src="<%=board.getBook().getCover() %>"></td>
	                    <th>제목</th>
...
%>
	        <a href="like.jsp?no=<%=boardNo %>&page=<%=pageNo %>" 
	            class="btn btn-outline-success position-relative <%=loginedUserNo == board.getUser().getUserNo() ? "disabled" : "" %>">추천
	            <i class="bi <%=showMyLike ? "bi-heart-fill" : "bi-heart" %>"></i>
	            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
	                <%=board.getLikeCount() %>
	            </span>
	        </a>
	        <a href="../qna/form-report.jsp?sno=<%=board.getUser().getNickname() %>" class="btn btn-outline-danger <%=loginedUserNo == board.getUser().getUserNo() ? "disabled" : "" %>">신고</a>
<%
 			} else {
%>
	        <a href="" class="btn tn-outline-secondary position-relative disabled">추천
	            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
	                <%=board.getLikeCount() %>
	            </span>
	        </a>
	        <a href="../qna/list-report.jsp" class="btn btn-outline-danger disabled">신고</a>
<%
 			}
%>
<%
...
%>
			<div class="float-end">
		        ...
		        <a href="list.jsp?<%=pageNo %>" class="btn btn-primary text-white">목록</a>
		    </div>
<% 
			} else { 
%>
	 		<div class="float-end">
				...
				<a href="list.jsp?page=<%=pageNo %>" class="btn btn-primary text-white">목록</a>
	 		</div>
<%
			}
%>
...
%>
                            ...
                            <a href="../qna/list-report.jsp" class="btn btn-outline-danger btn-sm disabled">신고</a>
                            <a href="" class="btn btn-success btn-sm disabled">수락 </a>
<%
				} else {
%>                      	
							<a class="btn btn-outline-dark btn-sm disabled">삭제</a>
							<a href="../qna/form-report.jsp?sno=<%=reply.getUser().getNickname() %>" class="btn btn-outline-danger btn-sm <%=userNo == -1 ? "disabled" : "" %>">신고</a>
							<a href="approve.jsp?rno=<%=reply.getNo() %>&bno=<%=board.getNo() %>&page=<%=pageNo %>" 
								class="btn btn-success btn-sm <%=userNo == board.getUser().getUserNo() && "판매중".equals(board.getStatus()) ? "" : "disabled" %>">수락
							</a>
<%
				}
%>							
