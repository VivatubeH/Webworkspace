 	...
 	<div>
		...
		%>
			<div class="text-end mt-3">
		        ...
		        <a href="list.jsp?page=<%=pageNo %>" class="btn btn-primary btn-sm">목록</a>
	    	</div>
		<% 
			} else {
		%>
			<div class="text-end mt-3">
		        <a href="update-form.jsp?no=<%=board.getNo() %>" class="btn btn-secondary btn-sm disabled">수정</a>
		        <a href="delete.jsp?no=<%=board.getNo() %>" class="btn btn-danger btn-sm disabled">삭제</a>
		        <a href="list.jsp?page=<%=pageNo %>" class="btn btn-primary btn-sm">목록</a>
	    	</div>
		<%
			}
	    ...
	    %>
				<a href="#" class="btn btn-outline-secondary position-relative"  onclick="alert('본인이 작성한 게시글은 추천할 수 없습니다.')">
		            추천
		            <i class="bi <%=showMyLike ? "bi-heart-fill" : "bi-heart" %>"></i>
		            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
		                <%=board.getLikeCount() %>
				...
				<a href="#" class="btn btn-outline-danger ms-2 " onclick="alert('본인이 작성한 게시글은 신고할 수 없습니다.')">신고</a>
	    <%
	        	}
	    	}
	    %>
