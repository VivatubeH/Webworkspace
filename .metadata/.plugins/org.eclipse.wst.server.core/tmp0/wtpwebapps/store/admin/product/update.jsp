...
<%--
	...
	처리내용
		...
		3. 2번에서 조회한 Product객체에 요청파라미터으로 조회된 값을 대입한다.
...
<%
	...
	// 4. 수정된 정보가 반영된 Product객체를 ProductDao객체의
	//    updateProduct() 메소드로 전달해서 데이터베이스에 반영시킨다.
	productDao.updateProduct(product);
	
	// 5. 해당 상품의 추가혜택정보를 전부 삭제한다.
	productDao.deleteBenefitsByProductNo(no);
	...
	if (benefitNoArr != null) {
		for (String value : benefitNoArr) {
			...
			productBenefit.setProductNo(no);
	...
	// 7. detail.jsp를 재요청하는 URL을 응답으로 보낸기
