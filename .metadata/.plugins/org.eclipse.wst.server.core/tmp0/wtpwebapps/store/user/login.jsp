...
<%--
	...
	처리내용
		...
		3. 비밀번호를 sha256으로 변환해서 2번에서 조회한 비밀번호와 비교한다.
			+ 비밀번호가 일치하지 않으면 login-form.jsp를 재요청
		4. HttpSession객체에 인증이 완료된 사용자 정보를 저장한다.
		5. index.jsp를 재요청한다.
--%>
<%
	// 1. 요청파라미터값을 조회한다.
	...
	if (user == null) {	// 아이디로 조회되는 사용자정보가 없음
		response.sendRedirect("login-form.jsp?invalid");
		return;
	}
	
	if ("Y".equals(user.getDisabled())) {	// 조회된 사용자는 비활성화상태임
	...
	if (!sha256Password.equals(user.getPassword())) { // 비밀번호 불일치
		response.sendRedirect("login-form.jsp?invalid");
		return;
	}
