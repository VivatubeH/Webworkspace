	...
	<table class="table">
		...
		<tbody>
			...
			<tr>
				...
				<th>댓글수</th>
				<td><%=Utils.toCurrency(board.getReplyCnt()) %></td>
			</tr>
			<tr>
				<th>등록일</th>
...
<%
		if (canReplyModify) {
%>
					<a href="mofify-reply-form.jsp" class="btn btn-outline-dark btn-sm">수정</a>
					<a href="delete-reply.jsp?rno=<%=reply.getNo() %>&bno=<%=board.getNo() %>&page=<%=pageNo %>" class="btn btn-outline-dark btn-sm">삭제</a>
<%		
		} else {
%>
					<a class="btn btn-outline-dark btn-sm disabled">수정</a>
